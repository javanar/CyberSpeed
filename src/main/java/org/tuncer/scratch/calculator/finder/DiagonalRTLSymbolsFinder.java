...
public class DiagonalRTLSymbolsFinder {

    private final Symbol[][] matrix;
    private final GameConfig gameConfig;

    public DiagonalRTLSymbolsFinder(final Symbol[][] matrix, final GameConfig gameConfig) {
        this.matrix = matrix;
    ...
    public Map<Symbol, WinCombination> find() {
        ...
        if (allEqual && !symbols.get(0).getType().equals(SymbolType.bonus)) {
            combinations.put(symbols.get(0), gameConfig.getWinCombinations().getSameSymbolsDiagonallyRightToLeft());
        }
